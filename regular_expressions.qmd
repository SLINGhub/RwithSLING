...
To selecting column contain specific text patterns use `matches()`. To select rows, replace and remove specific text in rows and cells use functions of the stringr package, which all start with str\_. using fu This function uses [regular expressions](https://cran.r-project.org/web/packages/stringr/vignettes/regular-expressions.html) to match column names. A few key elements in regular expression strings:
