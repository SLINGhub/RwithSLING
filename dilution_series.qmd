...
d_res <- d_rqc %>%
  ...
  mutate(
    ...
    mandel = map(data, function(x) DCVtestkit::calculate_mandel(x, "InjVol", "Area")),
    ppa = map(data, function(x) DCVtestkit::calculate_pra_linear(x, "InjVol", "Area")),
    tidy = map(models, function(x) broom::glance(x))) |> 
  unnest(c(tidy, mandel)) |> 
  dplyr::select(-data, -models)

# DCVtestkit::calculate_pra_linear, currently returns list instead vector
d_res$ppa <- d_res$ppa |> unlist()
...
ggplot(data = d_cer, aes(x = InjVol , y = Area,group = factor(DilutionSeries))) +
    ...
    stat_poly_eq(aes(label = after_stat(rr.label),
                     color = ifelse(after_stat(r.squared) < 0.80, "red", "darkgreen")),size = 2.4,lineheight = 1111.5) +
...
plot_page <- function(d, col_page, row_page, x_col, y_col, curve_col, feature_col) {
  ggplot(data = d, aes(x = {{x_col}} , y = {{y_col}}, group = factor({{curve_col}}))) +
     ...
     scale_color_identity() +
     scale_y_continuous(limits = c(0, NA))+
     facet_wrap(vars({{feature_col}}), scales = "free", nrow = row_page, ncol = col_page) +
     geom_point() + theme_light(base_size = 6)
}
...
plot_page(d_cer, col_page = 4, row_page = 5, x_col = InjVol, y_col = Area , curve_col = DilutionSeries, feature_col = Compound)

d <- d_rqc #|> filter(str_detect(Compound, "PC"))
